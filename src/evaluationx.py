...
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt  # <--- único import de plt
...
def _scores_from_estimator(estimator, X) -> np.ndarray:
    """Devuelve scores en [0,1] usando predict_proba si existe; si no, decision_function normalizada."""
...
def business_metrics(df: pd.DataFrame, y_true: np.ndarray, scores: np.ndarray, pred: np.ndarray,
    ...
    """
    expected_cost = base_cost + p_no*Pu + disputed*Pd + days_to_forward*Pf
    """
    p_yes = scores
    p_no  = 1.0 - scores
    ...
    summary = {
        "timely_rate_pred": float(np.mean(pred == 1)),
...
# Evaluation & plots
# ---------------------------
def evaluate_best(gs_dict: Dict[str, object], X_test, y_test, df_test: pd.DataFrame, do_plots: bool = True):
    """
    ...
    details = {}
    if do_plots:
        plt.close("all")  # 'plt' llega del import de nivel superior
